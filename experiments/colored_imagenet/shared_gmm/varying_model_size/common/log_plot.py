...
def log_and_plot_aggregated_metrics(
    ...
    if trained_models:
        ...
        if all("visualize_samples" in trained_models[size] for size in model_sizes):
            # Create directory for sample visualizations
            samples_dir = plot_dir / "sample_visualizations"
            samples_dir.mkdir(parents=True, exist_ok=True)
            
            for size in model_sizes:
                if "visualize_samples" in trained_models[size]:
                    samples = trained_models[size]["visualize_samples"]
                    if samples is not None:
                        # Create grid of samples
                        num_samples = len(samples)
                        if num_samples > 0:
                            # Determine grid size
                            grid_size = int(jnp.ceil(jnp.sqrt(num_samples)))
                            ...
                            plt.tight_layout()
                            save_path = samples_dir / f"generated_samples_{size}_means.png"
                            plt.savefig(save_path)
                            plt.close()
                            ...
                            if summary_logger:
                                summary_logger.log(
                                    {
                                        f"samples/model_size_{size}": wandb.Image(
                                            str(save_path.absolute())
                                        )
                                    },
                                    step=0,
    ...
    for loss_type in loss_types:  # train_loss or val_loss
        ...
        for t_idx in range(num_times):
            ...
            if baseline_models:
                ...
                for cat in ["ground_truth", "memorizing", "partial_mem"]:
                    valid_sizes = [
                    ...
                    if valid_sizes:
                        ...
                        if cat in ["ground_truth", "memorizing"]:
