...
experiment_group_name = f"data_gmm_model_shared_gmm_expt_varying_model_size_phase_transition_sweep_seed2"

experiment_group_dir = Path(__file__).parent / "results" / experiment_group_name
...
params_to_sweep = {
    "dim": [30, 40, 50, 60],
    "gt_num_components": [3, 6, 9, 12],
    "num_samples_train": [50, 100, 150, 200]
}

for param_set in itertools.product(*params_to_sweep.values()):
    dim, gt_num_components, num_samples_train = param_set

    experiment_name = experiment_group_name + f"_dim_{dim}_gt_num_components_{gt_num_components}_num_samples_train_{num_samples_train}"
    experiment_dir = experiment_group_dir / experiment_name
    ...
    train_skip = max(num_samples_train // 10, 1)
    eval_skip = max(num_samples_train // 100, 1)
    num_models_coverage = 10

    assert num_samples_train % train_skip == 0
    assert num_samples_train % eval_skip == 0

    num_components_to_train = list(range(train_skip, num_samples_train + 1, train_skip))
    num_components_to_evaluate = list(range(eval_skip, num_samples_train + 1, eval_skip))

    sampler_class = DDMSampler  # For RatioOfDistancesMetric

    num_steps_train = num_epochs_train * num_samples_train // batch_size
    num_steps_warmup = num_steps_train // 10
    ...
    plot_files = log_and_plot_aggregated_metrics(base_config_file, data_file, metrics_file, experiment_dir, use_wandb=True)

    # Save some memory
    # base_config_file.unlink()
    data_file.unlink()
    # for file in training_configs_files:
    #     file.unlink()
    # for file in baseline_configs_files:
    #     file.unlink()
    # for file in training_results_files:
    #     file.unlink()
    # for file in baseline_results_files:
    #     file.unlink()
